...
import { submitAnswer, completeInterview } from "../apis/mycvtracker/submit_interview";

// Temporarily
const TOKEN = "037ad5d5bc5842b08107656d3fb8efd9";
const INTERVIEW_TYPE = "typescript01";
...
const JobInterview = () => {
  ...
  const SkipQuestion = () => {
    if (window.confirm("Skip Question?")) {
      if (currentQuestionNo === questionsListLength) {
        window.alert("Thanks for attempting your Interview. Our team will get in touch soon.");
        FinishInterview();
        return;
      }
      setStartTimer(6);
  ...
  const SendAudioResponse = () => {
    let attempt_time = String((ansStopTime?.getTime() - ansStartTime?.getTime()) / 2);
    ...
    <>
      <div className={styles.container}>
        <span className={styles.message}>
          Please press the Start Interview <br /> button to begin the interview. <br /> Record your answer and upload{" "}
          <br /> each question to the server
          <br />
        </span>
        ...
        {type === 0 ? (
          <>
            <p className={styles.instruction}>
              ...
              <br />
              <span>
                Press Start Interview when you are ready to proceed with the interview questions, clicking record when
              ...
              <br />
              <span>
                You can playback your recording to determine if you are satisfied with the audio, and then ensure you
                upload your answer to the server to confirm your answer before moving to next Question.
              </span>
              <br />
              <span>
                Please note you need to record answer and submit answer at each question before moving to next question.
              </span>
              <br />
              <span>Continue this process until you have answered a minimum of 10 questions!</span>
            </p>
            <p className={styles.message_2}>Recording will start automatically</p>
          </>
        ) : startTimer !== 0 ? (
          <>
            <p className={styles.instruction}>{currentQuestion?.question}</p>
            <p className={styles.message_2}>Recording starts in {startTimer} seconds!</p>
          </>
        ) : !stopped ? (
          <>
            <p className={styles.instruction}>{currentQuestion?.question}</p>
            <p className={styles.message_3}>Recording started... {remainingTime} seconds left!</p>
          </>
        ) : (
          <>
            <p className={styles.instruction}>{currentQuestion?.question}</p>
            <p className={styles.message_4}>Recording Stopped. Please submit the audio or skip the question</p>
          </>
        )}
        {!stopped ? (
        ...
        {message === "Start Interview!!" ? (
          ...
          <div className={styles.btn_group}>
            {stopped && startTimer === 0 ? (
              <>
                <button className={styles.skip_btn} onClick={() => SkipQuestion()}>
                  Skip Question
                </button>
                <button className={styles.next_btn} onClick={() => NextQuestion(0)}>
                  Next Question
                </button>
              </>
            ) : (
        ...
        <div className={styles.submit_btn}>
          {CompleteRecording && (
            <button className={styles.next_btn} onClick={() => SubmitQuestion()}>
