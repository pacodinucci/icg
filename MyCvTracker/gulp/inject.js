...
gulp.task(
  ...
  function () {
    var injectStyles = gulp.src(
      [path.join(conf.paths.tmp, "/serve/app/main.css"), path.join("!" + conf.paths.tmp, "/serve/app/vendor.css")],
      { read: false }
    );
...
gulp.task("injectFront", ["stylesFront"], function () {
  return injectAlone({
    ...
    paths: [
      path.join(conf.paths.src, "/topcvreviews.html"),
      path.join(conf.paths.src, "/job-spec.html"),
      ...
      path.join(conf.paths.src, "/interviewResponse.html"),
      ...
      path.join(conf.paths.src, "/high-speed-hiring.html"),
      ...
      path.join(conf.paths.src, "/pdf-viewer.html"),
      path.join(conf.paths.src, "/network-share.html"),
      path.join(conf.paths.src, "/context-link.html"),
      path.join(conf.paths.src, "/cv-box.html"),
    ],
  });
});
...
var injectAlone = function (options) {
  var injectStyles = gulp.src(options.css, { read: false });
